...
- `index.html` — dashboard inteiro num arquivo só (Chart.js, logos em base64, dados em `RAW`).
  ...
  - Menu **Cadastro ▾ > ICMS** (aba `tab-cad-icms`, `cadastro-icms.js`): matriz UF origem × destino com a alíquota
    ...
    **Vigência = data FINAL** ("válida até", campo `vigencia`) + início opcional (`vigenciaInicio`). Escolha da tabela
    ...
    recente, depois o trecho mais específico, depois a que termina antes. `freteVigTxt(t)` formata para a tela.
    **Versões**: botão "🔁 Nova versão (reajuste)" na tabela (`tabCriarVersao`) copia trechos/regras com outro período e
    % de reajuste nos valores em R$ (não nos %); se a nova termina depois da atual, a atual termina na véspera do início
    da nova; se termina antes, a atual passa a começar no dia seguinte. Lista mostra Situação (Vigente/Encerrada/Futura)
    e agrupa versões pelo nome; a Auditoria mostra "calc.: tabela (período)". Itens removidos a pedido: Redespacho, KM rodado, Faixas de peso, Volume,
